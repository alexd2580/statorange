...
void echoPrimaryLemon(I3State* i3, SystemState* sys, Output* disp, uint8_t pos)
{
  ...
  {
    SEP_LEFT(neutral_colors);
    printf(" %s %s ", ethernet, sys->net_eth_ip);
  ...
  if(sys->net_wlan_up)
  {
    ...
    printf(" %s(%d%%%%) ", sys->net_wlan_essid, sys->net_wlan_quality);
    SEP_LEFT(neutral_colors);
    printf(" %s ", sys->net_wlan_ip);
    SEP_LEFT(white_on_black);
  }
  ...
  if(sys->bat_status != NO_BATTERY)
  {
    ...
    case BATTERY_CHARGING:
      SEP_LEFT(neutral_colors);
      printf(" BAT charging %ld%%%% ", 100*sys->bat_level/sys->bat_capacity);
      break;
    case BATTERY_FULL:
      SEP_LEFT(info_colors);
      printf(" BAT full ");
    ...
    case BATTERY_DISCHARGING:
    {
      ...
      printf(" BAT %ld%%%% ", 100*sys->bat_level/sys->bat_capacity);
      SEP_LEFT(current_set_colors);

      long rem_hrOnly = rem_minutes / 60;
      if(rem_hrOnly < 10) printf(" 0%ld", rem_hrOnly);
      else printf(" %ld", rem_hrOnly);

      long rem_minOnly = rem_minutes % 60;
      if(rem_minOnly < 10) printf(":0%ld ", rem_minOnly);
      else printf(":%ld ", rem_minOnly);
